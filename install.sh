...
#From VERSIONE xtract the English name of the release system in debian/ubuntu Add the corresponding Nginx apt source
...
is_root() {
    if [ 0 == $UID ]; then
        ...
        echo -e "${Error} ${RedBG} The current user is not root User, please switch to root Re-execute the script after the user ${Font}"
        exit 1
    fi
}
judge() {
...
chrony_install() {
    ...
    chronyc tracking -v
    date
    read -rp "Please confirm whether the time is accurate and the error range±3minute(Y/N): " chrony_install
...
web_camouflage() {
    ##Please note here and LNMP The default path of the script conflicts, do not Install NS LNMP Use this script in the environment, otherwise you will be responsible for the consequences
    rm -rf /home/wwwroot
...
v2ray_install() {
    ...
    mkdir -p /root/v2ray
    cd /root/v2ray || exit
    wget -N --no-check-certificate https://raw.githubusercontent.com/vinhvgs/V2Ray_ws-tls_bash_onekey/${github_branch}/v2ray.sh
...
nginx_exist_check() {
    if [[ -f "/etc/nginx/sbin/nginx" ]]; then
        echo -e "${OK} ${GreenBG} Nginx Already exists, skip compiling Install Process ${Font}"
        sleep 2
    elif [[ -d "/usr/local/nginx/" ]]; then
        echo -e "${OK} ${GreenBG} Other packages detected Install of Nginx，continue Install Will cause conflict, please deal with it Install${Font}"
        exit 1
    else
        nginx_install
    fi
...
nginx_install() {
    ...
    echo -e "${OK} ${GreenBG} About to start compiling Install jemalloc ${Font}"
    sleep 2
    ...
    judge "jemalloc Compile Install"
    echo '/usr/local/lib' >/etc/ld.so.conf.d/local.conf
    ldconfig

    echo -e "${OK} ${GreenBG} About to start compiling Install Nginx, It will take a little longer, please wait patiently ${Font}"
    sleep 4
    ...
    judge "Compile check"
    make -j "${THREAD}" && make install
    judge "Nginx Compile Install"

    # Revise basic configuration
...
ssl_install() {
    ...
    judge "Install SSL The certificate generation script depends on"
...
domain_check() {
    ...
    echo -e "${OK} ${GreenBG} Getting public network ip Information, please be patient ${Font}"
    local_ip=$(curl https://api-ipv4.ip.sb/ip)
    echo -e "domain name dns Parsing IP：${domain_ip}"
    echo -e "Native IP: ${local_ip}"
    sleep 2
    if [[ $(echo "${local_ip}" | tr '.' '+' | bc) -eq $(echo "${domain_ip}" | tr '.' '+' | bc) ]]; then
        echo -e "${OK} ${GreenBG} domain name dns Parsing IP With native IP match ${Font}"
        sleep 2
    else
        echo -e "${Error} ${RedBG} Please make sure the domain name is added with the correct A Record, otherwise it will not be able to use normally V2ray ${Font}"
        echo -e "${Error} ${RedBG} domain name dns Parsing IP With native IP Does not match whether to continue Install？（y/n）${Font}" && read -r install
        case $install in
        [yY][eE][sS] | [yY])
            ...
            ;;
        *)
            echo -e "${RedBG} Install termination ${Font}"
            exit 2
            ;;
        esac
    fi
...
v2ray_conf_add_tls() {
    cd /etc/v2ray || exit
    wget --no-check-certificate https://raw.githubusercontent.com/vinhvgs/V2Ray_ws-tls_bash_onekey/${github_branch}/tls/config.json -O config.json
    modify_path
    modify_alterid
    modify_inbound_port
    modify_UUID
...
v2ray_conf_add_h2() {
    cd /etc/v2ray || exit
    wget --no-check-certificate https://raw.githubusercontent.com/vinhvgs/V2Ray_ws-tls_bash_onekey/${github_branch}/http2/config.json -O config.json
    modify_path
    modify_alterid
    modify_inbound_port
    modify_UUID
...
EOF
    ...
    modify_nginx_port
    modify_nginx_other
    judge "Nginx Configuration Revise"
...
#}
acme_cron_update() {
    wget -N -P /usr/bin --no-check-certificate "https://raw.githubusercontent.com/vinhvgs/V2Ray_ws-tls_bash_onekey/dev/ssl_update.sh"
...
basic_information() {
    {
        ...
        echo -e "${Red} additionalid（alterId）：${Font} $(info_extraction '\"aid\"')"
        echo -e "${Red} additional（security）：${Font} 自适应 "
...
tls_type() {
    if [[ -f "/etc/nginx/sbin/nginx" ]] && [[ -f "$nginx_conf" ]] && [[ "$shell_mode" == "ws" ]]; then
        ...
        echo "请注意,如果你使用 Quantaumlt X / router / Old version Shadowrocket / Below 4.18.1 Version of V2ray core Please select compatibility mode"
        ...
        echo -e "${Error} ${RedBG} Nginx Or the configuration file does not exist or is currently Install Version is h2 ，Please be correct Install Execute after script ${Font}"
    fi
}
show_access_log() {
...
uninstall_all() {
    ...
    echo -e "${OK} ${Green} Whether to uninstall acme.sh And certificate [Y/N]? ${Font}"
...
update_sh() {
    ol_version=$(curl -L -s https://raw.githubusercontent.com/vinhvgs/V2Ray_ws-tls_bash_onekey/${github_branch}/install.sh | grep "shell_version=" | head -1 | awk -F '=|"' '{print $3}')
    ...
    if [[ "$shell_version" < "$(sort -rV $version_cmp | head -1)" ]]; then
        ...
        [yY][eE][sS] | [yY])
            wget -N --no-check-certificate https://raw.githubusercontent.com/vinhvgs/V2Ray_ws-tls_bash_onekey/${github_branch}/install.sh
...
menu() {
    ...
    echo -e "Currently Install Version:${shell_mode}\n"
    ...
    echo -e "${Green}10.${Font} Check V2Ray Configuration information"
    echo -e "—————————————— other options ——————————————"
    echo -e "${Green}11.${Font} Install 4合1 bbr Sharp speed Install script"
    ...
    2)
        ...
        ;;
    3)
        bash <(curl -L -s https://raw.githubusercontent.com/vinhvgs/V2Ray_ws-tls_bash_onekey/${github_branch}/v2ray.sh)
        ;;
    4)
